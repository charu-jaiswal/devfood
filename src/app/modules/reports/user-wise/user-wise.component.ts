...
import { UserWiseServices } from './user-wise.services';
...
@Component({
  ...
  styleUrls: ['./user-wise.component.css'],
})
export class UserWiseComponent implements OnInit {
  ...
  constructor(
    private userWiseServices: UserWiseServices,
  ...
  loadItemWise() {
    if (!this.loading) {
      ...
      this.userWiseServices.getReportAggUser(this.dataPost).subscribe((response: any) => {
