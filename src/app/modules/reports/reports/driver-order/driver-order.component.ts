...
import { ReportsFirePath, OutletFirePath, GlobalSettingFirePath, OutletCateFirePath, DriverFirePath } from '../../../firestore.path';
import { Reports, defaultReports, listCol, listSelectCol } from '../reports.model';
import { ReportListService } from './../reports-list/reports-list.services';
...
export class DriverOrderListComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    const FbDbSubscribe2 = this.fireDB.colWithIds$(`${DriverFirePath}`).subscribe((driverData: any) => {
      this.listDriver = driverData;
