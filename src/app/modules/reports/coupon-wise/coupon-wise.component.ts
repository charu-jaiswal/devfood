...
import { CouponWiseServices } from './coupon-wise.services';
import { Status, PaymentType, OrderType } from '../../../shared/common';
import { FirestoreService } from '../../../core/firestore.service';
import { ReportsFirePath, OutletFirePath, GlobalSettingFirePath } from '../../firestore.path';
...
@Component({
  selector: 'app-coupon-wise',
  templateUrl: './coupon-wise.component.html',
  styleUrls: ['./coupon-wise.component.css'],
})
export class CouponWiseComponent implements OnInit {
  timezone = 'GMT';
  outletID: any;
  listOutlets: any;
  listStatus: any;
  ...
  constructor(
    private couponWiseServices: CouponWiseServices,
  ...
  ngOnInit() {
    ...
    this.fireDB.colWithIds$(`${OutletFirePath}`).subscribe((response: any) => {
      ...
      if (this.outletID) {
        this.dataPost.outlet = this.route.pathFromRoot[this.route.pathFromRoot.length - 4].snapshot.paramMap.get('outletid');
      }
    });
  ...
  loadItemWise() {
    if (!this.loading) {
      ...
      this.couponWiseServices.getReportAggCoupon(this.dataPost).subscribe((response: any) => {
